...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	//把要发送的数据备份一份，放到send_buffer队尾
	send_buffer_ADD_TAIL(packet, len); 
	//打开定时器
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
    ...
    //单纯的ACK包可以不用重发
	if((flags != TCP_ACK) && ((flags & TCP_RST)==0) )
	{ 
		send_buffer_ADD_TAIL(packet, pkt_size);
		//打开定时器
		tcp_update_retrans_timer(tsk);
	}
...
void tcp_send_data(struct tcp_sock *tsk, char *buf, int tcp_data_len)
{
	...
	//注意，此处的send_packet会备份一份到send_buffer
